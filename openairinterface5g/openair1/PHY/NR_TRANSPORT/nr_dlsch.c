...
extern float oai_mac_get_dl_prb_util_avg(module_id_t mod_id);

static inline int oai_gnb_get_pdsch_amp(PHY_VARS_gNB *gNB) {
  // 런타임 전력(진폭) 읽기. 없으면 TX_AMP로 fallback
  return gNB->tx_amp_runtime;
}
...
static int do_one_dlsch(unsigned char *input_ptr, PHY_VARS_gNB *gNB, NR_gNB_DLSCH_t *dlsch, int slot)
{
  ...
  // const int      amp = oai_gnb_get_pdsch_amp(gNB);
  /* ---- Dynamic PDSCH power scaling ---- */
  float util = oai_mac_get_dl_prb_util_avg(0);   // gNB 0번 가정
  int   amp_i;
  if (util > 0.f && util < 0.20f)
    amp_i = 220;
  else
    amp_i = gNB->TX_AMP;
  const int16_t amp = (int16_t)(amp_i < -32768 ? -32768 : (amp_i > 32767 ? 32767 : amp_i));
  /* ------------------------------------- */
